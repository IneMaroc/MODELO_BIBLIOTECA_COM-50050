...
CREATE OR REPLACE VIEW VW_librosConA AS
	(SELECT DISTINCT titulo
    FROM libro
    WHERE titulo like 'A%');
    
SELECT * FROM VW_librosConA;

CREATE OR REPLACE VIEW VW_librosLargos AS
	(SELECT id_libro, titulo, cant_paginas AS cantidad_paginas
	FROM libro
    WHERE cant_paginas > (SELECT AVG(cant_paginas) FROM libro)
    ORDER BY titulo);
    
SELECT * FROM VW_librosLargos;

CREATE OR REPLACE VIEW VW_librosConAutor AS
	(SELECT li.id_libro, li.titulo as titulo, au.nombre_autor as autor
	FROM libro_autor as la
	JOIN libro as li ON la.id_libro=li.id_libro
    JOIN autor as au ON la.id_autor=au.id_autor
    WHERE nombre_autor like '%heidi%');
   
SELECT * FROM VW_librosConAutor;

CREATE OR REPLACE VIEW VW_librosJK AS
	(SELECT  count(li.id_libro) AS total_JKRowling
	FROM libro_autor as la
	JOIN libro as li ON la.id_libro=li.id_libro
    JOIN autor as au ON la.id_autor=au.id_autor
    WHERE nombre_autor like '%rowling%');
    
SELECT * FROM VW_librosJK;

CREATE OR REPLACE VIEW VW_librosConAutorEditorial AS
	(SELECT li.id_libro, li.titulo, au.nombre_autor AS autor, editorial.razon_social_edi AS editorial
	FROM libro_autor as la
	JOIN libro as li ON la.id_libro=li.id_libro
    JOIN autor as au ON la.id_autor=au.id_autor
    JOIN editorial ON li.id_editorial=editorial.id_editorial);
    
SELECT * FROM VW_librosConAutorEditorial;
...
DROP VIEW VW_librosConA;
DROP VIEW VW_librosJK;
DROP VIEW VW_librosLargos;
DROP VIEW VW_librosConAutor;
DROP VIEW VW_librosConAutorEditorial;
